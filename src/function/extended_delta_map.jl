...
function set_num_I_with_cmb!(set_params::SetParams)
    if set_params.which_model == "s1"
        set_params.num_I = 3
    elseif set_params.which_model == "d1"
        set_params.num_I = 4
...
function calc_clean_map_cmb(set_params::SetParams, fit_params::FitParams)
    npix = hp.nside2npix(set_params.nside)
    ...
    smoothed_map = smoothing_map_fwhm([0 * est_signal[1:npix], est_signal[1:npix], est_signal[npix+1:2npix]], fwhm_con, set_params.nside)
    clean_cmb_Q_sm = smoothed_map[2, :]
    clean_cmb_U_sm = smoothed_map[3, :]   
...
function calc_pix_based_chi_sq(set_params::SetParams, fit_params::FitParams, noise::Bool)
    ...
    return x_sm_masked' / cov_mat * x_sm_masked #, clean_Q, clean_U,
end
...
function calc_pix_based_likelihood(set_params::SetParams, fit_params::FitParams, noise::Bool)
    ...
    # Calculate the covariance matrix
    cov_mat = extract_masked_elements(set_params,  calc_all_cov_mat(set_params, fit_params) + art_noise_cov_mat)
    chi_sq = calc_pix_based_chi_sq(set_params, fit_params, noise)
