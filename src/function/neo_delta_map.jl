...
function D_list_set(set_params::SetParams, fit_params::FitParams)
    ...
    for nu_i in set_params.freq_bands
        ...
        if set_params.which_model == "s1" || set_params.which_model == "s5" 
            #D = [s, ss, sss]
            D = [s, ss]  # For s1 and s5, only s and ss are used    
        elseif set_params.which_model == "d1" || set_params.which_model == "d10" || set_params.which_model == "d0"
            #D = [d, dd, ddd, dddd, ddddd, dddddd]
            D = [d, dd, ddd]  # For d1 and d10, only d, dd, and ddd are used
            #D = [d, dd]
        elseif set_params.which_model == "d1 and s1" || set_params.which_model == "d10 and s5" || set_params.which_model == "d0 and s0"
            #D = [s, ss, sss, d, dd, ddd, dddd, ddddd, dddddd]
            D = [s, ss, d, dd, ddd]  # For d1 and s1, only s, ss, d, dd, and ddd are used
        end
        push!(D_list, hcat(D...))
    end
    return D_list
...
function set_truncate_TᵀN⁻¹_TᵀN⁻¹T!(set_params::SetParams, mask_path; tag::AbstractString)
    """
    Set TᵀN⁻¹, TᵀN⁻¹T
    """
    TᵀN⁻¹_set = []
